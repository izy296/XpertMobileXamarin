...
{
    public partial class UserSelector : PopupPage
    {

        public ContentPage pargentPage;

        UserSelectorViewModel viewModel;
        public string CurrentStream { get; set; }
        public bool searchEmpty { get; set; } = false;
        public string SearchedType { get; set; } = "";

        public UserSelector(string stream)
        {
            CurrentStream = stream;
            InitializeComponent();

            BindingContext = viewModel = new UserSelectorViewModel();

            MessagingCenter.Subscribe<MsgCenter, View_SYS_USER>(this, MCDico.ITEM_ADDED, async (obj, item) =>
            {
        ...
        private async void OnUpdateSwipeItemInvoked(object sender, EventArgs e)
        {
            ...
            var tr = (sender as SwipeItem)?.Parent?.Parent?.Parent?.BindingContext as View_SYS_USER;
