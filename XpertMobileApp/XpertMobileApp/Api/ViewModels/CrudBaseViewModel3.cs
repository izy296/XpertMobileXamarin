...
{
    public class CrudBaseViewModel3<T1, TView> : BaseViewModel
    where T1 : new()
    where TView : new()
    {
        ...
        internal async Task AddProducts(int index, int count)
        {
            if (IsBusy)
                return;
            ...
            try
            {
                ...
                var items = await service.SelectByPage(GetFilterParams(), index, count);
                Summaries.Clear();
        ...
        internal async void LoadMoreItems(object obj)
        {
            Syncfusion.ListView.XForms.SfListView listview = null;
            if (obj != null) 
            { 
                listview = obj as Syncfusion.ListView.XForms.SfListView;
                listview.IsBusy = true;
            }
            /*   
             *   await Task.Delay(2500);
            */
            var index = (Items.Count / PageSize) + 1;
            //var count = index + 3 >= totalItems ? totalItems - index : 3;
            await AddProducts(index, PageSize);
        ...
        internal bool CanLoadMoreItems(object obj)
        {
            if (Items.Count >= elementsCount)
                return false;
            return true;
        }
