    ...
    public class PrinterHelper
    {
        ...
        public async static void PrintBL(View_VTE_VENTE data)
        {
            ...
            try
            {
                ...
                printerLocal.setFont(0, 0, 0, 1, 0);
                printerLocal.PrintText("Vente N : " + data.NUM_VENTE + Environment.NewLine);
                printerLocal.PrintText("Client : " + data.NOM_TIERS + Environment.NewLine);
                string date = String.Format($"Date :{data.DATE_VENTE:dd/MM/yyyy}");
                ...
                printerLocal.PrintText("-----------------------------------------------" + Environment.NewLine);
                printerLocal.PrintText(string.Format($"Versement : {data.MBL_MT_VERCEMENT:0.00}" + $"        Total : {data.TOTAL_TTC:0.00}") + Environment.NewLine);
                printerLocal.PrintText(string.Format($"A.SOLDE : {data.OLD_SOLDE:0.00}") + Environment.NewLine);
                printerLocal.PrintText(string.Format($"N.SOLDE : {data.NEW_SOLDE:0.00}") + Environment.NewLine);
        ...
        public async static void PrintEntree(View_STK_ENTREE data)
        {
            if (data == null) return;
            try
            {
                ...
                printerLocal.PrintText("-----------------------------------------------" + Environment.NewLine);
                printerLocal.setFont(0, 0, 0, 1, 0);
                string date = String.Format($"Date :{data.DATE_ENTREE:dd/MM/yyyy}");

                printerLocal.PrintText("Retour Stock N : " + data.NUM_ENTREE + "      " + date + "" + Environment.NewLine);
                printerLocal.PrintText("Client : " + data.NOM_TIERS + Environment.NewLine);
                printerLocal.PrintText("Etablie par : " + data.CREATED_BY + Environment.NewLine);
                printerLocal.PrintText("Tel : " + user.TEL_USER + Environment.NewLine);
                printerLocal.PrintText("-----------------------------------------------" + Environment.NewLine);
                ...
                {
                    designation = item.DESIGNATION_PRODUIT != null && item.DESIGNATION_PRODUIT.Length > 18 ? item.DESIGNATION_PRODUIT.Substring(0, 18) : item.DESIGNATION_PRODUIT;
                    datvalue = string.Format($"{designation,-18} {item.QUANTITE,-5:N1} {item.PRIX_TTC,-10:0.00} {item.MT_TTC,-12:0.00}") + Environment.NewLine;
                    printerLocal.PrintText(datvalue);
                }
                printerLocal.PrintText("-----------------------------------------------" + Environment.NewLine);
                printerLocal.PrintText(string.Format($"                             Total : {data.TOTAL_TTC:0.00}") + Environment.NewLine);
        ...
        public async static void PrintEncaisseOffline(View_TRS_ENCAISS data)
        {
            ...
            try
            {
                ...
                printerLocal.setFont(0, 0, 0, 1, 0);
                string date = String.Format($"Date :{data.DATE_ENCAISS:dd/MM/yyyy}");

                printerLocal.PrintText("N : " + data.NUM_ENCAISS + "    " + date + Environment.NewLine);
                printerLocal.PrintText("Client : " + data.NOM_TIERS + Environment.NewLine);
                printerLocal.PrintText("Etablie par : " + data.CREATED_BY + Environment.NewLine);
                printerLocal.PrintText("-----------------------------------------------" + Environment.NewLine);
                if ("ENC".Equals(data.CODE_TYPE))
                ...
                printerLocal.PrintText($"A.Solde  : {data.OLD_SOLDE,-10:0.00}" + Environment.NewLine);
                printerLocal.PrintText($"N.Solde: {data.NEW_SOLDE,-10:0.00}" + Environment.NewLine);
